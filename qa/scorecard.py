...
def semgrep_findings(path: Path) -> dict:
    if not path.exists(): return {"high":0,"med":0,"low":0}
    data = json.loads(path.read_text())
    counts = {"high":0,"med":0,"low":0}
    for r in data.get("results", []):
        sev = (r.get("extra",{}).get("severity","") or "").lower()
        if sev.startswith("high"): counts["high"] += 1
        elif sev.startswith("med"): counts["med"] += 1
        else: counts["low"] += 1
    return counts
...
def main():
    ...
    # hard musts
    hard_fail = []
    if cov < hard["coverage_min"]: hard_fail.append(f"coverage {cov:.1f}% < {hard['coverage_min']}%")
